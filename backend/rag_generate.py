...
class RAGGenerator:
    ...
    def refine_query(self, query: str) -> str:
        ...
        """
        if self.config.refine_query == False:
            return query
    ...
    def _build_sources_metadata(self, results: list[RetrievalResult]) -> list[dict]:
        ...
        return seen #to accomodate api_server.py converting this function's output's values into a list.
    ...
    def _call_llm(self, query: str, context: str) -> str:
        ...
        if response.status_code != 200:
            ...
            raise Exception(f"OpenRouter API error: {error.get('message', 'Unknown error')}")
    ...
    def generate(self, query: str, top_k: Optional[int] = None, return_sources: bool = True) -> dict:
        ...
        pipeline = self.retrieval
        results = pipeline.retrieve(refined_query, top_k)
